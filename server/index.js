...
app.get('/api/quote-requests', authenticateAdmin, async (req, res) => {
  try {
    const result = await db.query('SELECT * FROM quote_requests ORDER BY id DESC');
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.delete('/api/quote-requests/:id', authenticateAdmin, async (req, res) => {
  const { id } = req.params;
  try {
    const result = await db.query('DELETE FROM quote_requests WHERE id = $1', [id]);
    if (result.rowCount === 0) {
